---
layout: page
title: Dr. Ting-Chun Lin
permalink: /team/lin/
nav: false
profile: true
---
<div class="row align-items-center">
    
<div class="col-sm-4 order-sm-last text-center mb-3 mb-sm-0">
        <img src="{{ site.baseurl }}/assets/img/team/TCLin.jpg" class="img-fluid rounded shadow-sm" alt="Ting-Chun Lin">
</div>

<div class="col-sm-8 order-sm-first">
<p class="text-justify">Dr. Ting-Chun Lin is a doctoral student (Dr. med.) at the University Bonn. With a clinical background in general medicine, she is currently exploring patient perspectives on the use of AI-based diagnostic tools, especially facial analysis systems, in routine clinical care.</p>
</div>
</div>

---

## Education

| Since 10.2020 | Postgraduate Training in General Medicine (Facharztweiterbildung Allgemeinmedizin), – Clinical experience in internal medicine, orthopedics, surgery, rehabilitation, psychosomatic medicine and family medicine according to WBO Berlin |
| 09.2019 | German Medical License (Approbation)  |
| 09.2010 - 06.2017 | M.D. in Human Medicine, National Yang Ming University, Taipei, Taiwan  |


---

## Certificates / Additional Training

| 02.2025 - Present | Basic Training in Acupuncture (A-Diploma), German Medical Acupuncture Association (DÄGFA), Berlin  |
| 07.2024 | Advanced Life Support, Charité University Hospital Berlin  |
| 03.2024 | Skin Cancer Screening, German Association of General Practitioners (Deutscher Hausärzteverband), Berlin  |
| 01.2024 | Interdisciplinary Basic Course in Sonography, German Society for Ultrasound in Medicine (DEGUM), Berlin  |
| 10.2023 - 11.2023 | Travel Medicine Certification, German Society for Tropical Medicine (DTG)  |
| 09.2022 - 11.2023 | Basic Psychosomatic Care, Vivantes Hospital Berlin  |
| 09.2019 | German Medical License (Approbation)  |

---

## Professional Memberships

- German Society for General and Family Medicine (DEGAM) 
- German Association of General Practitioners (Deutscher Hausärzteverband) 
- German Medical Acupuncture Association (DÄGFA) 
- Doctors Without Borders 
- Marburger Bund (German Physicians' Union)
